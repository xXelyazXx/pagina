...
export default function InscriptionForm() {
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      ...
      iframe.name = 'hidden_iframe_presentacion';
      document.body.appendChild(iframe);

      // Método HTML Form - el más confiable para Google Forms
      const form = document.createElement('form');
      form.method = 'POST';
      form.action = process.env.NEXT_PUBLIC_PRESENTACION_FORM_URL;
      form.target = 'hidden_iframe_presentacion'; // Enviar al iframe oculto
      form.style.display = 'none'; // Ocultar el formulario

      // Crear todos los campos como inputs hidden
      const fields = [
        { name: process.env.NEXT_PUBLIC_PRESENTACION_FORM_ENTRY_NOMBRE, value: data.fullName || '' }, // Nombre completo
        { name: process.env.NEXT_PUBLIC_PRESENTACION_FORM_ENTRY_CONTACTO, value: data.phone || '' }, // Teléfono
        { name: 'fbzx', value: process.env.NEXT_PUBLIC_PRESENTACION_FORM_FBZX } // Token de seguridad
      ];
      ...
      setTimeout(() => {
        toaster.create({
          ...
          description: "Su presentación ha sido enviada correctamente.",
          status: "success",
          duration: 5000,
        });
      }, 500);
      ...
      toaster.create({
        title: "Error al enviar la presentación",
      ...
      >
        ...
        >
          <Heading size="lg" mb={2} color={"black"}>
            Formulario de Presentación de niños
          </Heading>
          <Text fontSize="md" color={"black"}>
            Complete el siguiente formulario para presentars a sus niños
          </Text>
        </Box>
                ...
                render={({ field }) => (
                  ...
                  >
                    ...
                    <Checkbox.Label fontSize="sm" color="black">
                      Acepto que mis datos sean utilizados para procesar mi presentación{" "}
